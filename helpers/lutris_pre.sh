...
GAME_NAME_FILE="$SCRIPT_DIR/../data/game_monitor/current_game"

# Write the GAME_NAME to the designated file.
echo "$GAME_NAME" > "$GAME_NAME_FILE"

# Fetch the env from lutris
#env > "$SCRIPT_DIR/../data/game_monitor/lutris_prelaunch_env.txt"

# Log the launch event with additional information (optional).
#echo "Launching $GAME_NAME ($LUTRIS_GAME_UUID) [$STORE]" >> "$SCRIPT_DIR/../data/game_monitor/lutris_launch.log"
